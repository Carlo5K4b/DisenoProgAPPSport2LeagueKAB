...
namespace SportsLeague.Domain.Services
{
    public class MatchService : IMatchService
    {
        ...
        {
            var match2 = await _matchRepository.GetByIdAsync(id);

            if (match2 == null)
                throw new KeyNotFoundException($"No se encontró el partido con ID {id}");

            var validTransition = (match2.Status, newStatus) switch
            {
            ...
            if (!validTransition)
                throw new InvalidOperationException(
                    $"No se puede cambiar de {match2.Status} a {newStatus}");

            match2.Status = newStatus;

            _logger.LogInformation(
                "Updating match {MatchId} status to {NewStatus}", id, newStatus);
            await _matchRepository.UpdateAsync(match2);
        } 
    }
}
