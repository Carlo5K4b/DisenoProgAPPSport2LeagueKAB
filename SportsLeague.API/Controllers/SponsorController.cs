...
namespace SportsLeague.API.Controllers
{
    ...
    public class SponsorController : ControllerBase
    {
        ...
        public async Task<ActionResult<SponsorResponseDTO>> GetById(int id)
        {
            ...
            if (sponsor == null)
                return NotFound(new { message = $"Sponsor con ID {id} no encontrado en la base de datos" });
        ...
        // POST api/TournamentsSponsor/{id}
        [HttpPost("{id}/tournaments")]
        public async Task<ActionResult<TournamentSponsorResponseDTO>> RegisterTournament(
            int id, TournamentSponsorRequestDTO dto)
        {
            try
            {
                var tournamentSponsor = _mapper.Map<TournamentSponsor>(dto);
                var created = await _sponsorService.RegisterTournamentAsync(id, tournamentSponsor);
        ...
        [HttpDelete("{idSponsor}/tournaments/{idTournament}")]
        public async Task<ActionResult> RemoveFromTournament(int idSponsor, int idTournament)
        {
            try
            {
                await _sponsorService.RemoveFromTournamentAsync(idSponsor, idTournament);
                return NoContent();
            }
            catch (KeyNotFoundException ex)
            {
